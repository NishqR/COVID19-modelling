...
to make-persons
  ...
  create-persons num-people [
    ...
    set mask-roll random 100
    if mask-roll < mask-wearers [
      set mask? true
...
to go
  if num-days = 70 [ stop ]
  if all? persons [ immune? ] [ set policy-intervention? false ]
  if count persons with [ infected? = true ] = 0 [ set policy-intervention? false ]
...
to spread-infection
  ...
  ask persons with [ infected? ] [
    ...
    if ( count persons-here >= 1 ) and ( total-days-with-covid >= infectious-period ) and (total-days-with-covid < (incubation-period - infectious-days) + 10)[


      if wearing-masks? = true [

        if mask? = true [

          ask persons-here [
            if mask? = true and (random-float 100 + 1) < mask-mask-prob [infect-them]



            if mask? = false and (random-float 100 + 1) < mask-bare-prob [infect-them]
          ]
        ]
        ...
        if mask? = false[
            ...
            ask persons-here [
              if mask? = true and (random-float 100 + 1) < bare-mask-prob [infect-them]


              if mask? = false and (random-float 100 + 1) < bare-bare-prob [infect-them]
      ...
      if wearing-masks? = false [
        if (random-float 100 + 1) < transmission-chance [
          ask persons-here [
            infect-them
          ]
